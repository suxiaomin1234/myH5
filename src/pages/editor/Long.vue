        ...
        >
          <template v-for="(item, index) in sidebarMenus">
            <el-tab-pane
              ...
              :key="index"
              v-if="
                !item.pageMode || (item.pageMode && item.pageMode === pageMode)
              "
            >
              <el-tooltip
                slot="label"
...
export default {
  ...
  methods: {
     ...
     */
    async saveFn(item) {
      console.log('保存long', this.projectData)
      // let pages = this.projectData.pages;
      // for (let i in pages) {
      //   for (let n in pages[i].elements) {
      //     if (
      //       pages[i].elements[n].propsValue.names == "" &&
      //       pages[i].elements[n].isForm == true
      //     ) {
    ...
    async publishFn() {
      ...
      // 提交数据再预览
      if (!this.id) {
        this.$message.warning("请先保存当前长图文!");
      } else {
        this.$axios
