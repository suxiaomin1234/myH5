    ...
    <div class="preview-info-wrapper" v-if="!loading">
      <el-form label-width="90px" label-position="left">
        ...
        <el-form-item label="页面状态:">
          <span :class="[previewSt=='2' ? 'primary' : 'orange']">{{previewSt=='2' ? '已发布' : '未发布'}}</span>
        </el-form-item>
      </el-form>

      <div class="page-info">
        <div class="page-cover">
          <img :src="(shareData.coverImage || defaultCoverImage) | replaceIP" alt="">
        </div>
...
export default {
  ...
  computed: {
    pageLink() {
      return `${this.goUrl}/#/exhibite/${this.pageId}/${this.isTemplate}/${this.previewSt}`
  ...
  methods: {
    ...
    getData() {
      ...
      let apiUrl = ''
      if (this.previewSt == '1') {
        apiUrl = this.$interParams.h5Inters.pViewSceneInf
      } else if (this.previewSt == '2') {
        apiUrl = this.$interParams.h5Inters.sViewSceneInf
      } else {
        apiUrl = this.$interParams.h5Inters.previewScene
      }
      this.$axios
        .get(apiUrl + this.pageId)
        .then((res) => {
          this.loading = false
          if (res.code == '200') {
            this.pageData = JSON.parse(res.data ? res.data.data : '{}')
          } else {
