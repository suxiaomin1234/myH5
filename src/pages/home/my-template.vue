    ...
    <div class="page-content">
      ...
      <ul class="page-item-wrapper">
        <li class="page-item" v-for="(item, index) in pageList" :key="index">
          ...
          <div class="cover">
            <img :src="(item.coverPic || defaultCoverImage) | replaceIP" alt="">
          </div>
          ...
          <div class="page-item-data-pv border-T">
            <div class="btn-wrapper">
              <el-button type="text" size="mini" @click="editPage(item.id)">编辑</el-button>
            </div>
            <div class="btn-wrapper">
              <el-button type="text" size="mini" @click="copyPage(item.id)">使用模板</el-button>
            </div>
            <div class="btn-wrapper">
              ...
              <el-dropdown placement="top-start">
                ...
                <el-dropdown-menu slot="dropdown">
                  <el-dropdown-item>
                    <div @click="publishPage(item.id)">发布模板市场</div>
                  </el-dropdown-item>
                  <el-dropdown-item>
      ...
      <div class="page-paginate">
        <el-pagination background layout="prev, pager, next" :current-page.sync="searchParams.pageNo" :page-size="searchParams.pageSize" :total="searchParams.totalCount"
          @current-change="getPageList"></el-pagination>
      </div>
    </div>
    <notFundData v-if="!pageList.length" />
    <!--预览-->
    <previewPage v-if="showPreview" :pageId="previewId" :previewSt="previewSt" @closePreview="showPreview = false"></previewPage>
  </div>
</template>
  ...
  methods: {
    ...
    deletePage(id, index) {
      this.$confirm('此操作将永久删除该模板, 是否继续?', '提示', {
        ...
        this.loading = true
        this.$axios
          .delete(this.$interParams.h5Inters.deleteTemplate + id)
          .then((res) => {
            this.loading = false
            // 从页面删除
            if (res.code == '200') {
